...
class VisualizationModule(Module):
    def __init__(self, log_dir: pathlib.Path, args=None):
        ...
        # stream objects that will be opened in __enter__ and closed in __exit__  :
        self.sock = None
        self.preview_data = None #(self.files_dir / 'vis_data.txt').open(mode='w')
        self.pos_data = None #(self.files_dir / 'pos_data.txt').open(mode='w')
        self.repoints_data = None #(self.files_dir / 'repoints_data.txt').open(mode='w')

    def start(self):

        self.logger.info("Starting visualization module...")

        if self.args.visualize_locally:
            self.logger.warning("Visualization will run locally.")
            ...
            try:
                ...
                while True:
                    ...
                    if pos_last_ms is None:
                        ...
                        if pos_vis and self.get_time_ms() - vis_pos_last_ms > 1000/POS_PLOT_HZ and pos_vis["timestamp"] - pos_last_ms > 1000/POS_PLOT_HZ:
                            ...
                            if not self.args.save_visualization:
                                # Send ID first
                                self.sock.sendall(buf_id)
                                # Send the image length beforehand
                                self.sock.sendall(buf_len)
                                # Send data
                                self.sock.sendall(pos_buf)
                            else:
                    ...
                    if repoints_last_ms is None:
                        ...
                        if repoints and self.get_time_ms() - vis_repoints_last_ms > 1000/REPOINTS_PLOT_HZ and repoints["timestamp"] - repoints_last_ms > 1000/REPOINTS_PLOT_HZ:
                            ...
                            if not self.args.save_visualization:
                                ...
                                self.sock.sendall(buf_len)
                                # Send data
                                self.sock.sendall(repoints_buf)
                            else:
                    ...
                    if preview_last_ms is None:
                        ...
                        if ready_for_plot and self.get_time_ms() - 1000 - vis_preview_last_ms > 1000/PREVIEW_PLOT_HZ \
                            ...
                            if not self.args.save_visualization:
                                # Send ID first
                                self.sock.sendall(buf_id_b)
                                # Send the image length beforehand
                                self.sock.sendall(buf_len_b)
                                # Send data
                                self.sock.sendall(img_rs)
                            else:
                ...
                if self.sock is not None:
                    ...
                    self.sock.close()
                for fh in (self.preview_data, self.pos_data, self.repoints_data):
                    if fh is not None:
                        fh.close()
    ...
    def visualize_locally(self):
        while True:
            preview = self.get("drivers_module:preview")
            if preview:
                img = cv2.imdecode(np.frombuffer(preview["data"]["data"], dtype=np.int8), flags=cv2.IMREAD_COLOR)
