...
class VisualizationModule(Module):
    def __init__(self, log_dir: pathlib.Path, args=None):
        ...
        self.datastream_fps = 0.0
        self.imu_data_buffer_maxlen = 200
    ...
    def start(self):
        ...
        while True:
            ...
            if data_dict:
                ...
                if frm_idx == 1:
                    data_ms_time = int(data_dict["timestamp"])
                    ms_time = self.get_time_ms()

                if frm_idx > 10:
                    self.display_fps = frm_idx / ((self.get_time_ms() - ms_time)/1000.0)
                    self.datastream_fps = frm_idx / ((int(data_dict["timestamp"]) - data_ms_time)/1000.0)
                    frm_idx = 0
    ...
    def visualize_image_data(self, data: bytes) -> None:
        ...
        cv2.putText(decoded, f"Datastream fps: {self.datastream_fps}",
