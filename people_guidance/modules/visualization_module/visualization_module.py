...
class VisualizationModule(Module):
            ...
            while True:
                ...
                if pos_last_ms is None:
                    ...
                    if pos_vis and self.get_time_ms() - vis_pos_last_ms > 1000/POS_PLOT_HZ and pos_vis["timestamp"] - pos_last_ms > 1000/POS_PLOT_HZ:
                        ...
                        # Len of pos_data
                        buf_len = np.array([len(pos_buf)], dtype='uint32').tobytes()

                        # Encode id to uint8
                        buf_id = np.array([1], dtype='uint8').tobytes()
                ...
                if repoints_last_ms is None:
                    ...
                    if repoints and self.get_time_ms() - vis_repoints_last_ms > 1000/REPOINTS_PLOT_HZ and repoints["timestamp"] - repoints_last_ms > 1000/REPOINTS_PLOT_HZ:
                        ...
                        # Len of pos_data
                        buf_len = np.array([len(repoints_buf)], dtype='uint32').tobytes()

                        # Encode id to uint8
                        buf_id = np.array([2], dtype='uint8').tobytes()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.preview_data.close()
        self.pos_data.close()
        self.repoints_data.close()

    @staticmethod
    def resize_image(img):
        # Resize img
        rs = cv2.resize(img, PREVIEW_FRAMESIZE, interpolation=cv2.INTER_LINEAR)
        # Encode img to jpeg
